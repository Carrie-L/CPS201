...
//    fun getAllExhibitorsEN(): LiveData<List<Exhibitor>>
    ...
    /* =Search= */
//    @Query("select CompanyID,CompanyNameCN,CompanyNameTW,CompanyNameEN,BoothNo,HallNo,seqHall,IsFavourite,PhotoFileName,PYSimp from EXHIBITOR WHERE CompanyNameCN LIKE :text OR CompanyNameTW LIKE :text OR CompanyNameEN LIKE :text OR BoothNo LIKE :text order by PYSimp,SeqSC")
...
//
//    @Query("select CompanyID,CompanyNameCN,CompanyNameTW,CompanyNameEN,BoothNo,HallNo,seqHall,IsFavourite,PhotoFileName,StrokeEng from EXHIBITOR WHERE CompanyNameCN LIKE :text OR CompanyNameTW LIKE :text OR CompanyNameEN LIKE :text OR BoothNo LIKE :text order by StrokeEng,SeqEN")
//    fun querySearchEN(text: String): MutableList<Exhibitor>
    ...
    @Query("select CompanyID,CompanyNameCN,CompanyNameTW,CompanyNameEN,BoothNo,HallNo,seqHall,IsFavourite,PhotoFileName,PYSimp from EXHIBITOR WHERE CompanyID in (select CompanyID FROM CompanyProduct where CatalogProductSubID=:id ) order by PYSimp,SeqSC")
    fun getIndustryInExhibitorsSC(id: String): MutableList<Exhibitor>

    @Query("select CompanyID,CompanyNameCN,CompanyNameTW,CompanyNameEN,BoothNo,HallNo,seqHall,IsFavourite,PhotoFileName,StrokeTrad from EXHIBITOR WHERE CompanyID in (select CompanyID FROM CompanyProduct where CatalogProductSubID=:id ) order by CAST(StrokeTrad AS INT) ASC,CAST(SeqTC AS INT) ASC")
    fun getIndustryInExhibitorsTC(id: String): MutableList<Exhibitor>

    @Query("select CompanyID,CompanyNameCN,CompanyNameTW,CompanyNameEN,BoothNo,HallNo,seqHall,IsFavourite,PhotoFileName,StrokeEng from EXHIBITOR  WHERE CompanyID in (select CompanyID FROM CompanyProduct where CatalogProductSubID=:id ) order by StrokeEng,SeqEN")
    fun getIndustryInExhibitorsEN(id: String): MutableList<Exhibitor>

    /*  application in exhibitor */
    @Query("select CompanyID,CompanyNameCN,CompanyNameTW,CompanyNameEN,BoothNo,HallNo,seqHall,IsFavourite,PhotoFileName,PYSimp from EXHIBITOR WHERE CompanyID in (select CompanyID FROM CompanyApplication where IndustryID=:id ) order by PYSimp,SeqSC")
    fun getApplicationInExhibitorsSC(id: String): MutableList<Exhibitor>

    @Query("select CompanyID,CompanyNameCN,CompanyNameTW,CompanyNameEN,BoothNo,HallNo,seqHall,IsFavourite,PhotoFileName,StrokeTrad from EXHIBITOR WHERE CompanyID in (select CompanyID FROM CompanyApplication where IndustryID=:id ) order by CAST(StrokeTrad AS INT) ASC,CAST(SeqTC AS INT) ASC")
    fun getApplicationInExhibitorsTC(id: String): MutableList<Exhibitor>

    @Query("select CompanyID,CompanyNameCN,CompanyNameTW,CompanyNameEN,BoothNo,HallNo,seqHall,IsFavourite,PhotoFileName,StrokeEng from EXHIBITOR  WHERE CompanyID in (select CompanyID FROM CompanyApplication where IndustryID=:id ) order by StrokeEng,SeqEN")
    fun getApplicationInExhibitorsEN(id: String): MutableList<Exhibitor>

    /*  my exhibitor */
    @Query("select CompanyID,CompanyNameCN,CompanyNameTW,CompanyNameEN,BoothNo,HallNo,seqHall,IsFavourite,PhotoFileName,PYSimp from EXHIBITOR where IsFavourite=1  order by PYSimp,SeqSC")
    fun getMyExhibitorsSC(): LiveData<List<Exhibitor>>

    @Query("select CompanyID,CompanyNameCN,CompanyNameTW,CompanyNameEN,BoothNo,HallNo,seqHall,IsFavourite,PhotoFileName,StrokeTrad from EXHIBITOR where IsFavourite=1 order by CAST(StrokeTrad AS INT) ASC,CAST(SeqTC AS INT) ASC")
    fun getMyExhibitorsTC(): LiveData<List<Exhibitor>>

    @Query("select CompanyID,CompanyNameCN,CompanyNameTW,CompanyNameEN,BoothNo,HallNo,seqHall,IsFavourite,PhotoFileName,StrokeEng from EXHIBITOR where IsFavourite=1 order by StrokeEng,SeqEN")
